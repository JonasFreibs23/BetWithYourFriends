 ...
 */
namespace Sumtum\Autodoc;

use Sumtum\Autodoc\Extractor,
    Sumtum\Autodoc\View,
    Sumtum\Autodoc\View\JsonView,
    Sumtum\Autodoc\Exception;
...
class Builder
{
     ...
     */
    public function __construct(array $st_classes, $s_output_dir, $title = 'php-apidoc', $app_base_url, $s_output_file = 'index.html', $template_path = null)
    {
    ...
    protected function saveTemplate($data, $file)
    {
        ...
        $tr = array(
            '{{ content }}' => $data,
            '{{ title }}' => $this->_title,
            '{{ app_base_url }}' => $this->_app_base_url,
        ...
        if(!copy(__DIR__ . '/assets/js/jquery.base64.min.js', $this->_output_dir . 'assets/js/jquery.base64.min.js')){
          throw new Exception('Cannot copy javascript file to '.$this->_output_dir.'assets/js');
        }
        if(!copy(__DIR__ . '/assets/css/custom.css', $this->_output_dir . 'assets/css/custom.css')){
          throw new Exception('Cannot copy custom css file to '.$this->_output_dir.'assets/css');
        }
        if(!copy(__DIR__ . '/assets/css/bootstrap.min.css', $this->_output_dir . 'assets/css/bootstrap.min.css')){
          throw new Exception('Cannot copy bootstrap css file to '.$this->_output_dir.'assets/css');
        }
        if(!copy(__DIR__ . '/assets/js/bootstrap.min.js', $this->_output_dir . 'assets/js/bootstrap.min.js')){
          throw new Exception('Cannot copy bootstrap javascript file to '.$this->_output_dir.'assets/js');
        }
        if(!copy(__DIR__ . '/assets/js/jquery-1.10.2.min.js', $this->_output_dir . 'assets/js/jquery-1.10.2.min.js')){
          throw new Exception('Cannot copy jquery file to '.$this->_output_dir.'assets/js');
        }
        if(!copy(__DIR__ . '/assets/fonts/glyphicons-halflings-regular.eot', $this->_output_dir . 'assets/fonts/glyphicons-halflings-regular.eot')){
          throw new Exception('Cannot copy font file to '.$this->_output_dir.'assets/fonts');
        }
        if(!copy(__DIR__ . '/assets/fonts/glyphicons-halflings-regular.svg', $this->_output_dir . 'assets/fonts/glyphicons-halflings-regular.svg')){
          throw new Exception('Cannot copy font file to '.$this->_output_dir.'assets/fonts');
        }
        if(!copy(__DIR__ . '/assets/fonts/glyphicons-halflings-regular.ttf', $this->_output_dir . 'assets/fonts/glyphicons-halflings-regular.ttf')){
          throw new Exception('Cannot copy font file to '.$this->_output_dir.'assets/fonts');
        }
        if(!copy(__DIR__ . '/assets/fonts/glyphicons-halflings-regular.woff', $this->_output_dir . 'assets/fonts/glyphicons-halflings-regular.woff')){
          throw new Exception('Cannot copy font file to '.$this->_output_dir.'assets/fonts');
        }
        if(!copy(__DIR__ . '/assets/images/favicon.ico', $this->_output_dir . 'assets/images/favicon.ico')){
          throw new Exception('Cannot copy favicon to '.$this->_output_dir.'assets/images');
        }
    }
    ...
    protected function generateSandboxForm($st_params, $counter)
    {
        ...
        if (isset($st_params['ApiParams']) && is_array($st_params['ApiParams']))
        {
            ...
            {
                $params[] = strtr(static::$sandboxFormInputTpl, array('{{ name }}' => $param['name'], '{{ input_type }}' => $param['input_type'], '{{ description }}' => $param['description'], '{{ sample }}' => $param['sample']));
            }
        }
        ...
        if (isset($st_params['ApiHeaders']) && is_array($st_params['ApiHeaders']))
        {
            ...
            {
                $headers[] = strtr(static::$sandboxHeaderFormInputTpl, array('{{ name }}' => $header['name'], '{{ description }}' => $header['description'], '{{ sample }}' => $header['sample']));
            }
        }
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="col-md-12" style="overflow-x:auto">
                                <pre id="response_headers{{ elt_id }}">Response Details</pre>
                                <pre id="response{{ elt_id }}">Response Body</pre>
                            </div>
                        </div>
                    </div>
                </div><!-- #sandbox -->
...
{{ description }}
<hr>
<span style="font-family: Menlo,Monaco,Consolas,"Courier New",monospace !important;">{{ name }}</span>
...
<form enctype="application/x-www-form-urlencoded" role="form" action="{{ route }}" method="{{ method }}" name="form{{ elt_id }}" id="form{{ elt_id }}">
    ...
    <button type="reset" class="btn btn-danger reset" rel="{{ elt_id }}">Reset</button>
    <button type="button" class="btn btn-primary pull-right fetch_csrf" ref="{{ elt_id }}" id="csrf{{ elt_id }}">Get CSRF tokens</button>
</form></div>';

      static $sandboxFormInputTpl = '
<div class="form-group">
    <label class="control-label" for="{{ name }}">{{ name }}</label>
    <input type="{{ input_type }}" class="form-control input-sm" id="{{ name }}" placeholder="{{ description }} - Ex: {{ sample }}" name="{{ name }}"">
</div>';

      static $sandboxHeaderFormInputTpl = '
<div class="form-group">
    <label class="control-label" for="{{ name }}">{{ name }}</label>
    <input type="text" class="form-control input-sm" id="{{ name }}" placeholder="{{ description }} - Ex: {{ sample }}" name="{{ name }}" value="{{ sample }}"">
</div>';
}
